                        ...
                        <tr v-for="worker in users">
                            ...
                            <td colspan="3" style="text-align: center;">
                                <button type="button" class="btn btn-sm btn-warning"
                                        @click="editWorker(worker.id, worker.name)">
                                    <i class="glyphicon glyphicon-pencil"></i>
                                </button>

                                <button type="button" class="btn btn-sm btn-danger"
                                        @click="removeWorker(worker.id, worker.name)">
        ...
        <modal v-if="showWorkerModal" @close="showWorkerModal = false" v-cloak>
            ...
            <div slot="body">
                <div class="row dates-row">
                    ...
                    <div class="col-md-4">
                        <input type="text" id="worker_name_modal" class="form-control" name="worker_name"
                               v-model="worker_name">
                    </div>
                </div>
            </div>
